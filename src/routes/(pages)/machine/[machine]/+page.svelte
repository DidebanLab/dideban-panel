  ...
  <div class="w-full">
    <div
      ...
      {#each Object.keys(visibleSeries) as option}
        <button
          class="capitalize {visibleSeries[option] ? 'bg-[#fefefe]' : ''}"
  ...
  <div
    ...
    {#if visibleSeries.CPU}
      <div class="flex-1 h-full flex flex-col gap-1 justify-start items-start shadow-lg">
        ...
        <div class="w-full flex justify-center items-center">
          <StrokedGaugeChart
            value={MACHINE.cpu[MACHINE.cpu.length - 1].load_1}
            title="Load Avg (1m)" />
          <StrokedGaugeChart
            value={MACHINE.cpu[MACHINE.cpu.length - 1].load_5}
            title="Load Avg (5m)" />
          <StrokedGaugeChart
            value={MACHINE.cpu[MACHINE.cpu.length - 1].load_15}
            title="Load Avg (15m)" />
        </div>
      </div>
    {/if}
    {#if visibleSeries.Disk}
      <div class="flex-1 h-full flex flex-col gap-1 justify-start items-start shadow-lg">
        ...
        <div class="w-full grid grid-cols-2 justify-center items-center">
          <StrokedGaugeChart
            value={MACHINE.disk[MACHINE.disk.length - 1].usage_percent}
            title="Usage" />
    ...
    {#if visibleSeries.Memory}
      <div class="flex-1 h-full flex flex-col gap-1 justify-start items-start shadow-lg">
        ...
        <div class="w-full grid grid-cols-2 justify-center items-center">
          <StrokedGaugeChart
            value={MACHINE.memory[MACHINE.memory.length - 1].usage_percent}
            title="Usage" />
          <div class="w-full h-full flex justify-center items-center">
            <div
