<script>
  ...
  let error = $derived(status === 'error' || status === 'down');
  let warn = $derived(status === 'timeout');
  let ok = $derived(status === 'up');
...
<div
  ...
  <div class="flex justify-start items-center px-4.25 gap-4">
    ...
    <div class="flex flex-col justify-center items-start gap-1">
      <a href="/checkers/{checkId}" class="text-sm sm:text-lg dark:text-white">{name}</a>
