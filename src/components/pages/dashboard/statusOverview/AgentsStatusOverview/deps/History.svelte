<script>
  ...
  let { agentId, name, last_status, checked_at } = $props();
  let isOffline = $derived(last_status === 'offline');
  let lastChecked = $state(checked_at);
...
<div
  class="relative h-29 sm:h-32.75 border rounded-[14px] flex flex-col py-4 xl:py-6 gap-7 {isOffline
  ...
  <div class="flex justify-start items-center px-4.25 gap-4">
    <div
      class="hidden sm:flex justify-center items-center size-12 rounded-2xl {isOffline
        ? 'bg-[#EF4444]/10'
        : 'bg-[#00d492]/10'}">
      <svg
        version="1.1"
    ...
    <div class="flex flex-col justify-center items-start gap-1">
      <a href="/agents/{agentId}" class="text-sm sm:text-lg dark:text-white capitalize">{name}</a>
    ...
    <div class="flex justify-center items-center gap-2 ms-auto mb-auto">
      ...
      <div
        ...
        class:bg-[#ef4444]={isOffline}
        class:bg-[#00d492]={!isOffline}
        style="box-shadow: 0 0 10px 1px {isOffline ? '#ef4444' : '#00d492'};">
      </div>
    </div>
  </div>
  ...
  <div
    ...
    {#each history as detail (detail[0])}
      ...
      <button
        ...
        onmouseover={() => {
          http
            .get(endpoints.agentHistoryDetail(agentId, id))
        ...
        class="w-full h-4 rounded-[1px] hover:h-6 transition-all cursor-pointer relative group {error
          ? 'bg-[#EF4444]'
          : ok
            ? 'bg-green-700'
            : 'bg-[#FFFFFF]/5'}">
