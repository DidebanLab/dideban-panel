  ...
  <div class="flex flex-col justify-start items-start p-6 gap-6">
    ...
    <div class="flex flex-col w-full">
        ...
        <div class="relative w-15 flex">
          <input
            ...
            onblur={e => {
              ...
              if (Number.isNaN(value) || value < AGENT_LIMIT.timeoutSeconds.min)
                form.timeout = AGENT_LIMIT.timeoutSeconds.min;
              else if (value > AGENT_LIMIT.timeoutSeconds.max) form.timeout = AGENT_LIMIT.timeoutSeconds.max;
            }}
          ...
          <div
           ...
           flex flex-col justify-center items-center gap-2.5 h-full">
            <button
              type="button"
              onclick={() => form.timeout < AGENT_LIMIT.timeoutSeconds.max && (form.timeout += 1)}
            ...
            <button
              type="button"
              onclick={() => form.timeout > AGENT_LIMIT.timeoutSeconds.min && (form.timeout -= 1)}
              class="size-0.5 flex items-center justify-center
             text-gray-500 hover:text-gray-300 scale-75 cursor-pointer">
              ▼
            </button>
      ...
      <div class="flex justify-between items-center text-sm pt-2">
        <div class="flex justify-start items-center gap-1">
          <span class="flex justify-center items-center mr-1 text-black dark:text-white">Interval</span>
          <span class="flex justify-center items-center text-gray-400/40">(seconds)</span>
        </div>
        ...
        <div class="relative w-15 flex">
          <input
            ...
            onblur={e => {
              ...
              if (Number.isNaN(value) || value < AGENT_LIMIT.intervalSeconds.min)
                form.interval = AGENT_LIMIT.intervalSeconds.min;
              else if (value > AGENT_LIMIT.intervalSeconds.max) form.interval = AGENT_LIMIT.intervalSeconds.max;
            }}
            class="px-3 h-6.5 w-full bg-[#0D0D0D]/5 dark:bg-white/5 backdrop-blur-sm rounded-md
           text-gray-400 text-sm outline-none tracking-wide appearance-none text-center" />
          ...
          <div
           ...
           flex flex-col justify-center items-center gap-2.5 h-full">
            <button
              type="button"
              onclick={() => form.interval < AGENT_LIMIT.intervalSeconds.max && (form.interval += 1)}
              class="size-0.5 flex items-center justify-center
             text-gray-500 hover:text-gray-300 scale-75 cursor-pointer">
              ▲
            </button>
            ...
            <button
              type="button"
              onclick={() => form.interval > AGENT_LIMIT.intervalSeconds.min && (form.interval -= 1)}
              class="size-0.5 flex items-center justify-center
             text-gray-500 hover:text-gray-300 scale-75 cursor-pointer">
              ▼
            </button>
    ...
    <button
      ...
      type="button"
      class="mt-10 me-auto w-fit px-10 text-sm text-[#10b981] h-8.5 flex justify-center items-center rounded-md cursor-pointer bg-[#22c55e]/10 hover:opacity-60 border border-[#00bc7d]/10 disabled:opacity-50 disabled:dark:opacity-30 disabled:cursor-not-allowed">
      Add Agent
    </button>
  </div>
</div>
