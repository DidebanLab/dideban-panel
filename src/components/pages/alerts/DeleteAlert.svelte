<script>
  ...
  let nameInput = $state('');
  let nameRef = $state();
  let { name, id, onDelete = () => {}, relType } = $props();
...
<div
  ...
  <div class="flex flex-col sm:flex-row justify-between items-end w-full gap-2 sm:gap-10">
    <div class="flex flex-col justify-start items-start gap-2 pt-2 w-full">
      <span class="text-black dark:text-white text-base">Delete This Alert</span>
      {#if step === 1}
        <span class="text-[#90A1AF] text-sm"
          >Once you delete a Alert, there is no going back. Please be certain.
        </span>
      {:else if step === 2}
        <span class="text-[#90A1AF] text-sm"
        ...
        <input
          ...
          placeholder={`Please enter the name of ${relType}`}
    ...
    <button
      ...
      onclick={async () => {
        if (step === 1) {
          ...
          http.delete(endpoints.singleAlert(id)).then(res => {
            alertStore.addAlert({
              message: `Alert of ${relType} ${name} deleted.`,
              type: 'successful',
            });
