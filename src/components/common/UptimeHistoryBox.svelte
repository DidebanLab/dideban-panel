...
<div
  class="flex flex-col  gap-3 p-3 sm:p-4 pt-3 dark:md:bg-[#0D0D0D] md:bg-[#FFFFFF] border border-[#0D0D0D]/5 dark:border-white/5 rounded-lg">
  <div
  ...
  <div class="grid grid-cols-7 gap-2 md:gap-3 justify-start items-start backdrop-blur-3xl rounded-lg">
    {#each data as item}
      <button
        type="button"
        ...
        class="size-3 sm:size-3.5 rounded-xs sm:rounded-sm hover:animate-pulse cursor-pointer {item < 70
          ? 'bg-red-700'
          : item < 80
            ? 'bg-[#c0590f]'
            : 'bg-green-700'}">
